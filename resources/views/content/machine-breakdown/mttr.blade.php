...
@section('title', 'MTTR (Mean Time To Repair')
@section('page-title', 'MTTR Performance Trend')
...
<div class="space-y-6" x-data="{ uploadOpen: false, showTableMatrix: false }">
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-2xl bg-slate-900 border border-slate-800 shadow-xl">
    <div>
      <div class="flex items-center gap-2">
        ...
        <h2 class="text-lg font-bold text-white tracking-wide">MTTR (MEAN TIME TO REPAIR)</h2>
      </div>
      <p class="text-xs text-slate-400 mt-1">
        Rata-rata durasi perbaikan mesin per bulan dari 19 line mesin (Khusus status Result) &bull; Target Tetap: <strong class="text-rose-400">{{ number_format($targetConstant, 0, ',', '.') }} Menit</strong>
      </p>
    </div>

    <div class="flex items-center flex-wrap gap-3 text-xs">
      <!-- Year Selector Form -->
      <form method="GET" action="{{ route('machine-breakdown.mttr') }}" class="flex items-center gap-2">
        <label for="yearSelect" class="text-slate-400 font-medium">Tahun Fiskal:</label>
        <select
          name="year"
          id="yearSelect"
          onchange="this.form.submit()"
          class="bg-slate-950 border border-slate-800 text-white font-semibold rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500"
        >
  ...
  @if (session('success'))
    <div class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs flex items-center justify-between">
      <div class="flex items-center gap-2">
  ...
  <div
    ...
    class="p-5 rounded-2xl bg-slate-900 border border-slate-800 shadow-xl"
    style="display: none;"
  >
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        <div class="md:col-span-3">
          ...
          <input
            ...
            id="file"
            name="file"
            class="block w-full text-xs text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-indigo-600 file:text-white hover:file:bg-indigo-500 bg-slate-950 border border-slate-800 rounded-xl cursor-pointer p-1"
            accept=".xlsx,.xls,.csv"
            required
          >
        </div>
        <div class="md:col-span-1">
          <button type="submit" class="w-full py-2.5 px-4 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition shadow-lg shadow-indigo-600/30">
            Upload & Proses
          </button>
        </div>
      </div>
      <p class="text-[11px] text-slate-500">
        * Sistem mengimpor data dan menghitung rata-rata durasi penanganan bulanan (MTTR) dari 19 line mesin (khusus status Result).
      </p>
    </form>
  </div>
  @endhasPermission
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    <!-- Card 1: Rata-Rata MTTR -->
    <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 shadow-md">
      <div class="text-xs text-slate-400 font-medium uppercase tracking-wider mb-1">Rata-Rata MTTR FY {{ $fiscalYearLabel }}</div>
      <div class="flex items-baseline gap-2">
        <span class="text-3xl font-extrabold text-indigo-400 font-mono">{{ number_format($avgMttr, 2, ',', '.') }}</span>
        <span class="text-xs text-slate-400">menit</span>
      </div>
      <div class="mt-2 text-[11px] text-slate-500">Rata-rata durasi perbaikan mesin aktif</div>
    </div>

    <!-- Card 2: Target MTTR -->
    <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 shadow-md">
      <div class="text-xs text-slate-400 font-medium uppercase tracking-wider mb-1">Target MTTR (Ketentuan)</div>
      <div class="flex items-baseline gap-2">
        <span class="text-3xl font-extrabold text-rose-400 font-mono">{{ number_format($targetConstant, 0, ',', '.') }}</span>
        <span class="text-xs text-slate-400">menit</span>
      </div>
      <div class="mt-2 text-[11px] text-slate-500">Garis batas target maksimal tetap</div>
    </div>

    <!-- Card 3: Status Pencapaian -->
    <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 shadow-md">
      <div class="text-xs text-slate-400 font-medium uppercase tracking-wider mb-1">Status Kecepatan Perbaikan</div>
      <div class="mt-1">
        @if ($avgMttr <= $targetConstant)
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
            &check; Memenuhi Target (&le; 12)
          </span>
        @else
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-rose-500/10 text-rose-400 border border-rose-500/20">
            &excl; Melebihi Target (&gt; 12)
          </span>
        @endif
      </div>
      <div class="mt-2 text-[11px] text-slate-500">
        Selisih: <strong class="{{ $avgMttr <= $targetConstant ? 'text-emerald-400' : 'text-rose-400' }}">{{ ($avgMttr > $targetConstant ? '+' : '') . number_format($avgMttr - $targetConstant, 2, ',', '.') }}</strong>
      </div>
    </div>
    ...
    <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 shadow-md">
      ...
      <div class="text-lg font-bold text-white font-mono mt-1">April {{ $selectedYear }} - Maret {{ $selectedYear + 1 }}</div>
    </div>
  </div>

  <!-- 5. MAIN MTTR CHART: LINE WITH MARKER -->
  <div class="p-5 rounded-2xl bg-slate-900 border border-slate-800 shadow-xl">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-4 mb-4 border-b border-slate-800 gap-2">
      <div>
        <h3 class="text-base font-bold text-white flex items-center gap-2">
          <span>MTTR Performance Trend (Line with Marker)</span>
          <span class="px-2 py-0.5 rounded text-[11px] font-mono bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">
            FY {{ $fiscalYearLabel }}
          </span>
        </h3>
        <p class="text-xs text-slate-400 mt-0.5">Grafik garis dengan marker titik untuk rata-rata durasi perbaikan per bulan (19 line) vs target 12 menit</p>
      </div>

      <!-- Legend Indicator & Click Hint -->
      <div class="flex items-center flex-wrap gap-3 text-xs">
        <div class="flex items-center gap-1.5 text-slate-200">
          <span class="w-3 h-3 rounded-full bg-indigo-500 border-2 border-white inline-block"></span>
          <span class="font-semibold">MTTR Actual (Rata-rata 19 Line)</span>
        </div>
        <div class="flex items-center gap-1.5 text-slate-200">
          <span class="w-4 h-0.5 border-t-2 border-dashed border-rose-500 inline-block"></span>
          <span class="font-semibold text-rose-400">Target (12)</span>
        </div>
        <div class="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 text-[11px]">
          <svg class="w-3.5 h-3.5 text-indigo-400 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="relative w-full" style="height: 380px;">
      <canvas id="mttrMarkerChart"></canvas>
    </div>
  </div>

  <!-- 6. GRAFIK RINCIAN MTTR PER LINE / AREA MESIN (SUMBU X: LINE, SUMBU Y: MENIT) -->
  <div id="lineDetailChartSection" class="p-5 rounded-2xl bg-slate-900 border border-slate-800 shadow-xl scroll-mt-6">
    <div class="flex flex-col lg:flex-row lg:items-center justify-between pb-4 mb-4 border-b border-slate-800 gap-3">
      <div>
        <div class="flex items-center gap-2">
          ...
          <h3 class="text-base font-bold text-white flex items-center gap-2">
            <span>Grafik MTTR per Line / Area</span>
            <span id="selectedMonthBadge" class="px-2.5 py-0.5 rounded text-xs font-mono bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">
              <!-- Diisi otomatis oleh JavaScript -->
            </span>
          </h3>
        </div>
        <p class="text-xs text-slate-400 mt-1">
          Sumbu X: Line / Area Mesin (Pareto: Terbesar ke Terkecil) &bull; Sumbu Y: Durasi MTTR (Menit) &bull; Garis Merah Putus-putus: Target (12 Menit)
        </p>
      </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
      <div class="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
        ...
        <div class="text-lg font-bold text-indigo-400 font-mono mt-0.5" id="monthAvgStat">0,00 m</div>
      </div>
      <div class="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
      ...
      <div class="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
        <div class="text-[11px] text-slate-400 uppercase tracking-wider">Memenuhi Target (&le; 12)</div>
        <div class="text-lg font-bold text-emerald-400 font-mono mt-0.5" id="monthAchievedCount">0 / 19</div>
      </div>
      <div class="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
        <div class="text-[11px] text-slate-400 uppercase tracking-wider">Over Target (&gt; 12)</div>
        <div class="text-lg font-bold text-rose-400 font-mono mt-0.5" id="monthOverCount">0 / 19</div>
      </div>
    </div>

    <!-- Canvas Grafik Detail Line MTTR -->
    <div class="relative w-full" style="height: 430px;">
      <canvas id="lineMttrDetailChart"></canvas>
    </div>
    ...
    <div x-show="showTableMatrix" x-cloak class="mt-4 overflow-x-auto rounded-xl border border-slate-800">
      <table class="w-full text-left text-xs text-slate-300">
        <thead class="bg-slate-950 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-800">
          <tr>
            <th class="py-3 px-3 min-w-[200px] sticky left-0 bg-slate-950 z-10">AREA</th>
        ...
        <tbody class="divide-y divide-slate-800/50 font-mono text-[11px]">
          @foreach ($matrixRows as $row)
            <tr class="hover:bg-slate-800/50 transition">
              <td class="py-2.5 px-3 font-sans font-semibold text-slate-200 sticky left-0 bg-slate-900 z-10 border-r border-slate-800/80">
                {{ $row['line'] }}
              </td>
              @foreach ($matrixColumns as $col)
                @php
        ...
        <tfoot class="border-t-2 border-indigo-500/40 font-mono text-xs font-bold bg-slate-950/80">
          <tr class="bg-indigo-500/10">
            <td class="py-3 px-3 font-sans uppercase tracking-wider sticky left-0 bg-slate-950 z-10 text-amber-400 border-r border-slate-800">
              AVERAGE
            </td>
            @foreach ($matrixColumns as $col)
              @php
              ...
              @endphp
              <td class="py-3 px-2.5 text-center {{ $avgVal !== null ? 'text-emerald-400 text-[12px] font-extrabold' : 'text-slate-500' }}">
                {{ $avgVal !== null ? number_format($avgVal, 2, ',', '.') : '0,00' }}
              </td>
            @endforeach
          </tr>
          <tr class="bg-rose-500/10 border-t border-slate-800">
            <td class="py-2.5 px-3 font-sans uppercase tracking-wider sticky left-0 bg-slate-950 z-10 text-rose-400 border-r border-slate-800">
              TARGET
            </td>
            @foreach ($matrixColumns as $col)
  ...
  <!-- 7. TABEL RINGKASAN EVALUASI BULANAN VS TARGET 12 -->
  <div class="rounded-2xl bg-slate-900 border border-slate-800 shadow-xl overflow-hidden">
    <div class="p-4 border-b border-slate-800 flex items-center justify-between">
      <h3 class="text-sm font-bold text-white">Ringkasan Evaluasi Bulanan vs Target 12 Menit</h3>
      ...
      <table class="w-full text-left text-xs text-slate-300">
        <thead class="bg-slate-950 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-800">
          <tr>
            ...
            <th class="py-3 px-4 text-right">Rata-Rata Durasi (MTTR)</th>
        ...
        <tbody class="divide-y divide-slate-800/60 font-mono">
          @foreach ($tableData as $item)
            <tr class="hover:bg-slate-800/40 transition">
              ...
              <td class="py-3 px-4 text-right font-bold {{ $item['value'] !== null ? ($item['value'] <= $item['target'] ? 'text-emerald-400' : 'text-indigo-400') : 'text-slate-600' }}">
              ...
              <td class="py-3 px-4 text-center font-sans">
                ...
                @elseif ($item['value'] <= $item['target'])
                  <span class="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">Achieved</span>
                @else
                  <span class="px-2 py-0.5 rounded text-[10px] font-bold bg-rose-500/10 text-rose-400 border border-rose-500/20">Above Target</span>
                @endif
              </td>
            </tr>
          @endforeach
...
@push('scripts')
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      const mttrSeries = @json(array_values($mttrSeries));
      ...
      // =========================================================================
      // 1. GRAFIK UTAMA: RATA-RATA MTTR BULANAN (12 BULAN FISKAL)
      // =========================================================================
      const canvas = document.getElementById('mttrMarkerChart');
      if (!canvas) return;
        ...
        data: {
          ...
          datasets: [
            // 1. DATASET MTTR ACTUAL
            {
              ...
              borderWidth: 2.5,
              pointStyle: 'circle',
              pointRadius: mttrSeries.map(v => v !== null && v > targetConstant ? 7 : 6),
              pointHoverRadius: 9,
              pointBackgroundColor: mttrSeries.map(v => v !== null && v > targetConstant ? '#f43f5e' : '#6366f1'),
              pointBorderColor: mttrSeries.map(v => v !== null && v > targetConstant ? '#fecdd3' : '#0f172a'),
              pointBorderWidth: 2,
              fill: false,
              tension: 0.1,
              spanGaps: false,
              ...
              datalabels: {
                ...
                color: function(context) {
                  ...
                  return (val !== null && val > targetConstant) ? '#fda4af' : '#c7d2fe';
                },
                font: {
          ...
          onClick: function(event, elements) {
            if (elements && elements.length > 0) {
              ...
              if (targetSection) {
                ...
                targetSection.classList.add('ring-2', 'ring-indigo-400');
                setTimeout(() => {
                  targetSection.classList.remove('ring-2', 'ring-indigo-400');
                }, 1200);
              }
            }
          },
      ...
      // =========================================================================
      // 2. GRAFIK DETAIL PER LINE / AREA UNTUK BULAN TERPILIH (MTTR)
      // Sumbu X: Line / Area Mesin | Sumbu Y: Durasi MTTR (Menit)
      // =========================================================================
      const lineCanvas = document.getElementById('lineMttrDetailChart');
      ...
      function renderLineChart(monthIndex) {
        ...
        // Styling warna: merah jika > 12 menit (over target MTTR), indigo/cyan jika <= 12 menit (achieved)
        const bgColors = lineValues.map(v => v > targetConstant ? 'rgba(244, 63, 94, 0.85)' : 'rgba(99, 102, 241, 0.85)');
        const hoverBgColors = lineValues.map(v => v > targetConstant ? 'rgba(225, 29, 72, 1)' : 'rgba(79, 70, 229, 1)');
        const borderColors = lineValues.map(v => v > targetConstant ? '#f43f5e' : '#6366f1');
        ...
        const avgVal = monthlyAverages[monthNum] !== undefined && monthlyAverages[monthNum] !== null ? monthlyAverages[monthNum] : null;
        if (avgEl) avgEl.innerText = avgVal !== null ? formatNumber(avgVal) + ' m' : '0,00 m';
        ...
        if (peakEl) peakEl.innerText = peakVal > 0 ? `${peakLine} (${formatNumber(peakVal)} m)` : '-';
        ...
        document.querySelectorAll('.month-pill').forEach((pill, idx) => {
          if (idx === monthIndex) {
            pill.className = 'month-pill px-2.5 py-1 rounded-lg text-xs font-semibold transition bg-indigo-600 text-white shadow ring-1 ring-indigo-400';
          } else {
        ...
        if (!lineDetailChart) {
          lineDetailChart = new Chart(lineCtx, {
            data: {
              labels: lineNames,
              datasets: [
                // 1. Line Target (12 Menit)
                {
                  type: 'line',
                  label: 'Target (12 Menit)',
                  data: targetValues,
                  borderColor: '#ef4444',
                  backgroundColor: '#ef4444',
                  borderDash: [6, 4],
                ...
                // 2. Bar MTTR tiap Line
                {
                  type: 'bar',
                  label: 'MTTR Line (Menit)',
                  data: lineValues,
                  ...
                  datalabels: {
                    ...
                    formatter: (value) => formatNumber(value),
                    color: function(context) {
                      return Number(context.dataset.data[context.dataIndex]) > targetConstant ? '#fda4af' : '#c7d2fe';
                    },
                    font: {
